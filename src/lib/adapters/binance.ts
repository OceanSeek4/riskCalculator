...
export const binance: ExchangeAdapter = {
  ...
  async fetchTicker(symbol, type): Promise<Ticker> {
    ...
    try {
      ...
      console.warn(`Binance fetchTicker failed, using fallback for ${sym}:`, error)
  ...
  async fetchKlines(symbol, type, interval, limit) {
    ...
    try {
      ...
      console.warn(`Binance fetchKlines failed, using fallback for ${sym}:`, error)
  ...
  async fetchMarketMeta(symbol, type): Promise<MarketMeta> {
    ...
    try {
      ...
      console.warn(`Binance fetchMarketMeta failed, using fallback for ${sym}:`, error)
      const fallback = getStaticMarketMeta('BINANCE', sym, type)
      if (!fallback) {
        throw new Error(`Symbol ${sym} not supported on Binance ${type} (offline mode)`)
      }
      return fallback
    }
  },
